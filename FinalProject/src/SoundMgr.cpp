...
bool SoundMgr::initWatercraftSounds(){
        ...
        for(std::map<int, Entity381 *>::const_iterator et = engine->entityMgr->GetEntities()->begin(); et != engine->entityMgr->GetEntities()->end(); ++et)
        	{
            //this->registerBattleSound(et, battleFilename);
            if (true){
                if (et->second->auioId == 1){
                        this->registerSelection(*(et->second), selection2Filename);
                }
                else{
                        this->registerSelection(*(et->second), selectionFilename);
                }
...
void SoundMgr::syncListenerToCamera(){
	...
	Ogre::Vector3 cameraPosition = mSceneMgr->getSceneNode("MainCamera")->getPosition();
	...
	Ogre::Quaternion q = mSceneMgr->getSceneNode("MainCamera")->getOrientation();
...
void SoundMgr::attachSelectedNodeToSoundIndex(Entity381 *ent, unsigned int index){
	...
	Ogre::Vector3 pos = ent->mPosition;
...
void SoundMgr::tick(double dtime){
	...
	syncListenerToCamera();
        ...
        //selection sound
		for(std::map<int, Entity381 *>::const_iterator it = engine->entityMgr->GetEntities()->begin(); it != engine->entityMgr->GetEntities()->end(); ++it){
           if (it->second == engine->entityMgr->GetSelectedEntity() && !it->second->didSelectSoundPlay){
        	   playSelectionSound(*(it->second));
        	   it->second->didSelectSoundPlay = true;
           }
           else if (it->second != engine->entityMgr->GetSelectedEntity() && it->second->didSelectSoundPlay){
        	   it->second->didSelectSoundPlay = false;
           }
        }
}
